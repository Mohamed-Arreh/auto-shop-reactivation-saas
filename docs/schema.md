...
Multi-tenant Postgres schema with Row-Level Security. Every tenant-scoped table includes `shop_id` and is protected by RLS policies that filter by the current user's shop membership.

## Design Principles

- **Multi-tenancy first** — every tenant-scoped table has `shop_id` and RLS enforcement
- **Provider-agnostic** — data sources, messaging, AI, and billing all flow through abstraction layers (see `architecture.md`)
- **Line items as the central work unit** — declined work is a status on a line item, not a separate table
- **Historical accuracy preserved** — original line items keep their `declined` status forever; recovery tracked separately
- **Versioned voice configs** — shop voice changes preserve history for rollback
- **Auditable** — `audit_log` and `compliance_records` tables capture state changes for compliance and debugging

## Note on Billing Fields

Fields like `subscription_status` and `subscription_tier` are internal labels managed by the operator via Stripe. Customers do not select or see plans inside the application — pricing is negotiated in sales conversations and billing is initiated via operator-generated Stripe Checkout links. The app's role is to track subscription state and respond to Stripe webhooks.
...
Every tenant-scoped table has RLS enabled with policies that filter by the current user's shop membership:

```sql
create policy "shop_members_access_<table>" on <table>
  for all
  using (
    shop_id in (
...
Service role bypasses RLS for system jobs (cron, webhooks, attribution).
